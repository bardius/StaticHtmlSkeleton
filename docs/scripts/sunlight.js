        ...
        return {
            ...
            handle_ident: function(context) {
                ...
                return (
                    ...
                    evaluate(context.language.namedIdentRules.follows, function(ruleData) {
        ...
        return function(tokens) {
            ...
            for (j = 0; j < tokenRequirements.length; j++) {
                ...
                if (actual === undefined) {
                    ...
                    (expected["values"] === undefined || contains(expected.values, actual.value, caseInsensitive))
                ) {
                    //derp
                    continue;
    ...
    Highlighter.prototype = (function() {
            ...
            function parseScopes(context) {
                ...
                for (tokenName in context.language.scopes) {
                    ...
                    for (j = 0; j < specificScopes.length; j++) {
                        ...
                        if (
                            opener !== value &&
                            (!context.language.caseInsensitive || value.toUpperCase() !== opener.toUpperCase())
                        ) {
                            continue;
                        }
        ...
        function tokenize(unhighlightedCode, language, partialContext, options) {
            ...
            if (partialContext.continuation) {
                ...
                tokens.push(
                    continuation(context, continuation, "", context.reader.getLine(), context.reader.getColumn(), true)
                );
            }
            ...
            var nodes = [],
                prepareText = (function() {
                    ...
                    if (options.showWhitespace) {
                        ...
                        tab =
                            new Array(options.tabWidth).join(String.fromCharCode(0x2014)) + String.fromCharCode(0x2192);
                    } else {
        ...
        return {
            ...
            highlightNode: function highlightRecursive(node) {
                var match, languageId, currentNodeCount, j, nodes, k, partialContext, container, codeContainer;
                ...
                if (getComputedStyle(node, "display") === "block") {
                    ...
                    if (this.options.maxHeight !== false) {
                        ...
                        codeContainer.style.maxHeight =
                            this.options.maxHeight + (/^\d+$/.test(this.options.maxHeight) ? "px" : "");
                    }
